...
3.2. 从客户端向服务端发送三个包，在TCP和UDP协议的情况下，服务端最终可能会收到几个包？

- 从**连接**性质来说：TCP需要握手，UDP不需要握手。
- 从**可靠性**来说：TCP丢包会重传，UDP不会；（当然不会一直重传，TCP重传次数有15次，如果还不行就reset连接了）
- 从**分片**来说：TCP根据协商的MSS来分片；UDP没有分片，则依赖IP层的分片；

3.3. 服务器内存开始不足时，对服务器上运行的进程有哪些影响？

- 操作系统会将一些进程的**内存页交换**到磁盘上，给其他进程腾出空间。这样导致进程性能下降；
- 操作系统可能会强制终止进程（OOM）；

3.4. 分析sql语句的执行计划，其中uuid和userid都是索引，sql语句为：select * from tableA where uuid >= 1000 and uuid <= 2000 order by userid

3.5. 记录服务请求的观测数据比如：<method、请求耗时>；每分钟会对所有服务观测结果做一次聚合统计，比如<method、平均请求耗时，总请求次数>。给出你的方案，并评估当QPS为10W时该统计方案对服务性能的影响。
